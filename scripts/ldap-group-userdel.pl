...
sub display_help {
         print "
usage: $0  -g|--group=adminservers  -t|--tree=Domain  -u|--user=abcd  [-l|--listoptions]  [-h|--help] [-v|--version]
...
if ( is_group_vmware ($handle, $group, $opt{'groupbase'})) {
        $vmware = 1;
}

if ( group_has_user ( $handle, $group, $opt{'groupbasedn'}, $user, $opt{'userbasedn'}) ) {
        if ( ! group_del_user ($handle, $group, $opt{'groupbasedn'}, $user, $opt{'userbasedn'} ) )
        {
                &logit($logfile, "SUCCESS: group_del_user() group => $group ".$opt{'groupbasedn'} . ", user => " . $opt{'userbasedn'} );
        } else {
                print ("ERROR: group_del_user() group $group ".$opt{'groupbasedn'}.", user => " . $opt{'userbasedn'} . "\n");
                &logit($logfile,"ERROR: group_del_user() group $group ".$opt{'groupbasedn'}.", user => " . $opt{'userbasedn'} );
                &cleanup;
                exit 1;
        }
}
...
if ( user_has_description ($handle, $user,  $opt{'userbasedn'}, $opt{'groupbasedn'} ) ) {
        &logit ( $logfile, "INFO: user_has_description() description=" . $opt{'groupbasedn'} . " exists from user " . $opt{'userbasedn'} );
        if ( ! user_remove_description ($handle, $user, $opt{'userbasedn'}, $opt{'groupbasedn'} ) )
        {
                print ( "ERROR: user_remove_description() userdn => ".$opt{'userbasedn'}.", description => " . $opt{'groupbasedn'} . "\n" );
                &cleanup;
                exit 1;
        }

} else {
        &logit ( $logfile, "INFO: user_has_description() description=" . $opt{'groupbasedn'} . " missing for user " . $opt{'userbasedn'} );
}
...
sub is_group_vmware {
        ...
        $msg = $ldap->search ( base => $where,
                               filter => "(&(objectclass=top)(objectclass=groupOfUniqueNames)(cn=$name))"
        );
...
sub group_has_user {
        ...
        &logit ($logfile, "SUCCESS: group_has_user() Checked for filter $filter in group $grpbase");
...
sub group_del_user {
        ...
        if (!defined ($vmware)) {
                # regular posix
                $msg = $ldap->modify($grpbasedn, delete => { memberUid => "$user", hMemberDN => "$userdn" } );
        } else {
                $vm = 1;
                # vmware groupOfUniqueNames
                $msg = $ldap->modify($grpbasedn, delete => { uniqueMember => "$userdn" } );
        }

        if ($msg->code) {
                print ("ERROR: group_sel_user() group => $grpbasedn, user => $user, hMemberDN => $userdn, memberUid => $user, uniqueMember => $userdn, VM => $vm reason: $msg->error");
                &logit ($logfile, "ERROR: group_del_user() group => $grpbasedn, user => $user, hMemberDN => $userdn, memberUid => $user, uniqueMember => $userdn, VM => $vm reason: $msg->error");
                return 1;
        } else {
                &logit ($logfile, "SUCCESS: group_del_user() group => $grpbasedn, user => $user, hMemberDN => $userdn, uniqueMember => $userdn, VM => $vm");
                return 0;
        }
