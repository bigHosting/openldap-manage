...
if ( (scalar(@ARGV < 0)) || (!defined($sshPublicKey)) || (!defined($tree)) || (!defined($user)) ) {  &display_help; }
...
if ($objectclass)
{
        ...
        foreach my $key (@ssh_keys)
        {
                ...
                if ( grep /^${sshkey_sign}$/, @ssh_keys ) {
                         print "[*] $0: ERROR: duplicate key [$key] found!\n";
                         &logit ( $logfile, "ERROR: duplicate key [$key] found for user " . $opt{'userbasedn'} );
                         &cleanup;
                         exit(1);
                }
        }
        ...
        # add new ssh key
        if ( ! &add_key ($handle, $user, $opt{'userbasedn'}, $numkeys ) )
        {
                &logit ( $logfile, "SUCCESS: userdn => ".$opt{'userbasedn'} . ", ssh key =>  " . $sshPublicKey );
        } else {
                print ( "ERROR: add_key() userdn => ".$opt{'userbasedn'}.", ssh key => " . $sshPublicKey . "\n" );
                &cleanup;
                exit(1);
        }
...
# no objectclass
        ...
        # add objectclass + ssh key
        if ( &add_objectclass_and_key ($handle, $user, $opt{'userbasedn'} ) )
        {
                &logit ( $logfile, "SUCCESS: add_objectclass_and_key() userdn => ".$opt{'userbasedn'} . ", ssh key =>  " . $sshPublicKey );
                print ( "SUCCESS: userdn => ".$opt{'userbasedn'} . ", ssh key =>  " . $sshPublicKey . "\n" );
        } else {
                print ( "ERROR: add_objectclass_and_key() userdn => ".$opt{'userbasedn'}.", ssh key => " . $sshPublicKey . "\n" );
                &cleanup;
                exit(1);
        }
}
...
sub add_key {
        ...
        $msg = $ldap->modify($userdn, add => {
                                               sshPublicKey => "$sshPublicKey"
                                             }
                            );
        if ($msg->code)
        {
                print ("ERROR: add_key() user => $userdn, reason: $msg->error");
                &logit ($logfile, "ERROR: add_key() user => $userdn, sshPublicKey => $sshPublicKey reason: $msg->error");
                return 1;
        } else {
                &logit ($logfile, "SUCCESS: add_key() user => $userdn, sshPublicKey => $sshPublicKey");
        }
...
sub add_objectclass_and_key {
        ...
        if ($msg->code)
        {
                print ("ERROR: add_objectclass_and_key() user => $userdn, reason: $msg->error");
                &logit ($logfile, "ERROR: add_objectclass_and_key() user => $userdn, sshPublicKey => $sshPublicKey reason: $msg->error");
                return 0;
        } else {
                &logit ($logfile, "SUCCESS: add_objectclass_and_key() user => $userdn, sshPublicKey => $sshPublicKey");
        }
